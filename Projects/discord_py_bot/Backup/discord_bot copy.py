...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.idle, activity=discord.Game('Young hoes'))
...
async def on_message(message):
    ...
    # if message.content in ['COD','cod','Cod','Cod?','COD?']:
    # if message.content in [x for x in cod_names]:
    # if any(x in message.content for x in cod_names):
        # await message.channel.send(random.choice(cod_pharses))

    if 'cod' in message.content.lower():
        await message.channel.send(random.choice(cod_pharses))

    # if message.content in ['hello','Hello','hey','HEY','hi','say','Hi']:
    # # if any(x in message.content for x in sayings):
    ...
    # if message.content in ['fuck','Fuck','FUCK','shit','my','no','yes','Yes','chris']:
    # # if any(x in message.content for x in sayings_josh):
    ...
    # if message.content in ['Monty','monty','nani','snake','python','hack','code','josh']:
    # # if any(x in message.content for x in monty_list):
...
async def server(ctx, *, text='I see the greatness currently online!'):
    staff = get(ctx.guild.roles, name="admin")
